...
// ­ƒùô´©Å Agenda PRO OdontoCloud ÔÇô impresi+¦n en ventana emergente + exportaci+¦n con cabecera de empresa
...
/* =========================
   Marca / impresi+¦n / export
   ========================= */
const COMPANY_NAME = "TU EMPRESA S.A.S.";             // <- cambia aqu+¡
const COMPANY_LOGO_URL = "/assets/logo-empresa.png";  // <- cambia aqu+¡ (ruta p+¦blica)
...
// normaliza cadenas (para b+¦squedas consistentes con nombre_busqueda)
const normalize = (s) =>
  (s || "")
    .toString()
    .toLowerCase()
...
export default function Agenda() {
  ...
  /* ======== NUEVO: posicionar agenda seg+¦n la URL al montar/cambiar ======== */
  useEffect(() => {
    ...
    if (target) {
      ...
      setSelectedDate(day);
      setMonthDate(day);
      setViewMode("day"); // aseguramos vista d+¡a al aterrizar
      try {
        // si ven+¡a de sessionStorage como fallback, limpiamos
      ...
      (snapshot) => {
        const allItems = snapshot.docs.map((docu) => {
          ...
          return {
            ...
            doctor: data.doctor || data.doctorNombre || "ÔÇö",
  ...
  // ------------ Navegaci+¦n fecha principal (texto) ------------
  const currentDateLabel = useMemo(() => {
    ...
    return "+Ültimas citas (detalle)";
  }, [viewMode, selectedDate]);

  // ------------ Controles fecha (igual que ten+¡as) ------------
  const goPrev = () => {
...
<head>
  <meta charset="utf-8">
  <title>Imprimir ÔÇô ${safe(companyName)}</title>
  <style>${css}</style>
</head>
<body>
  <div class="hdr">
    ...
    <div>
      ...
      <div class="sub">Agenda de citas -À ${safe(currentDateLabel)}</div>
    </div>
  </div>
  ...
  <script>
    // Imprimir cuando termine de cargar la pesta+¦a (incluye el logo)
  ...
  // Crear Blob y abrir en nueva pesta+¦a (evita el "about:blank" vac+¡o)
  const blob = new Blob([html], { type: "text/html;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const w = window.open(url, "_blank", "noopener,noreferrer,width=900,height=700");

  if (!w) {
    alert("El navegador bloque+¦ la ventana de impresi+¦n. Permite pop-ups para este sitio.");
    URL.revokeObjectURL(url);
    return;
  }

  // Liberar URL cuando cargue o despu+®s de un tiempo de seguridad
  ...
  const handleConfirmar = () =>
    alert("Funci+¦n de confirmar citas se integrar+í con n8n / correo.");
  ...
  const handleDeleteCita = async (cita) => {
    ...
    try {
      ...
      alert("ÔØî No se pudo eliminar la cita.");
    }
  };

  /* =========================
     Nueva Cita (igual que tu l+¦gica, intacta)
     ========================= */
  const handleNuevaCita = () => {
    ...
    box.innerHTML = `
      <h2 style="margin-top:0;text-align:center;color:#0a86d8;font-weight:600;">­ƒùô´©Å Nueva Cita</h2>
      <form id="formNuevaCita" style="display:flex;flex-direction:column;gap:12px;margin-top:10px;">
        ...
        <small style="color:gray;">Si el paciente no existe, marca ÔÇ£Registrar nuevo pacienteÔÇØ.</small>
        ...
        <div id="nuevoPacienteCampos" style="display:none;flex-direction:column;gap:8px;margin-top:10px;padding:10px;border-radius:8px;background:#f9f9f9;border:1px solid #ddd;">
          ...
          <div style="display:flex;gap:5px;">
            <select id="npIndicativo" style="width:40%;">
              <option value="+57">­ƒç¿­ƒç¦ +57</option>
              <option value="+1">­ƒç¦­ƒç© +1</option>
              <option value="+34">­ƒç¬­ƒç© +34</option>
              <option value="+52">­ƒç¦­ƒç¢ +52</option>
              <option value="+54">­ƒçª­ƒçÀ +54</option>
              <option value="+593">­ƒç¬­ƒç¿ +593</option>
              <option value="+56">­ƒç¿­ƒç¦ +56</option>
              <option value="+58">­ƒç+­ƒç¬ +58</option>
            </select>
        ...
        .map(
          (p, i) => `
            <div class="ac-item" data-index="${i}" data-id="${p.id}">
              ...
              <span class="ac-doc">-À ${p.documento || "s/doc"}</span>
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (!fechaSeleccionada || !horaSeleccionada) {
        alert("ÔÜá´©Å Debes seleccionar fecha y hora para la cita.");
        return;
      }
      if (!doctorSel) {
        alert("ÔÜá´©Å Indica el doctor asignado.");
        return;
      }
      ...
      try {
        ...
        if (esNuevo) {
          ...
          if (!pacienteNombre) {
            alert("ÔÜá´©Å Debes indicar nombre y apellido para el nuevo paciente.");
            return;
          }
          ...
          if (!celularNuevo && !telefonoNuevo) {
            alert("ÔÜá´©Å Debes registrar al menos un tel+®fono (celular o fijo) del paciente o de un acudiente.");
            return;
          }
          ...
          if (seleccionado) {
            ...
            if (!nombreLibre) {
              alert("ÔÜá´©Å Por favor, escribe o selecciona un paciente.");
              return;
            }
            ...
            if (!sDoc.empty) {
              ...
              if (encontrado) {
                ...
                pacienteNombre = nombreLibre; // +¦ltimo fallback
              }
            }
          }
        }
        ...
        if (!telefonoFinal) {
          alert("ÔÜá´©Å Este paciente no tiene ning+¦n tel+®fono registrado. Edita sus datos y agrega un n+¦mero antes de guardar la cita.");
          return;
        }
        ...
        alert("Ô£à Cita registrada correctamente y vinculada al paciente.");
        modal.remove();
      } catch (err) {
        console.error("ÔØî Error al guardar cita:", err);
        alert("ÔØî Error al guardar la cita. Revisa la consola para m+ís detalles.");
      }
    });
  };
  ...
  const handleEditCita = (cita) => {
    ...
    box.innerHTML = `
      <h2 style="margin-top:0;text-align:center;color:#0a86d8;font-weight:600;">Ô£Å´©Å Editar Cita</h2>
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (!paciente) return alert("ÔÜá´©Å El nombre del paciente es obligatorio.");
      if (!fecha || !hora) return alert("ÔÜá´©Å Debes indicar fecha y hora.");
      ...
      try {
        ...
        alert("Ô£à Cita actualizada correctamente.");
        modal.remove();
      } catch (err) {
        console.error("ÔØî Error al actualizar cita:", err);
        alert("ÔØî Error al actualizar la cita. Revisa la consola para m+ís detalles.");
      }
    });
  };
  ...
  const renderRow = (cita) => {
    ...
    const fechaHoraDisplay = `${fechaStr} -À ${horaStr}`;
          ...
          <aside className="odc-left" aria-label="Panel izquierdo - calendario y filtros">
            ...
            <div className="odc-filters-card" aria-label="Filtros de agenda">
              ...
              <select id="filterSucursal" value={filterSucursal} onChange={(e) => setFilterSucursal(e.target.value)}>
                <option value="">ÔÇö Todas ÔÇö</option>
              ...
              <select id="filterDoctor" value={filterDoctor} onChange={(e) => setFilterDoctor(e.target.value)}>
                <option value="">ÔÇö Todos ÔÇö</option>
          ...
          <main className="odc-right" aria-label="Panel principal - agenda">
            ...
            >
              <div className="odc-header-left" style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                ...
                {/* FECHA: al mismo nivel que las pesta+¦as y el buscador */}
                <div className="date-nav" style={{ display: "flex", alignItems: "center", gap: 6, flex: "0 0 auto" }}>
                  <button className="btn icon" id="btnPrev" aria-label="Anterior" onClick={goPrev}>ÔÇ¦</button>
                  <button className="btn icon" id="btnNext" aria-label="Siguiente" onClick={goNext}>ÔÇ¦</button>
            ...
            <div className="odc-content-card" role="region" aria-label="Contenido agenda">
              <div className="tools-row">
                ...
                <div className="tools-right">
                  <button id="btnPrint" className="btn" title="Imprimir agenda" onClick={handlePrint}>­ƒû¿´©Å Imprimir</button>
                  <button id="btnConfirmar" className="btn blue" title="Confirmar citas" onClick={handleConfirmar}>­ƒôº Confirmar</button>
                  <button id="btnHoyCitas" className="btn" title="Ver citas de hoy" onClick={handleHoyCitas}>­ƒôà Hoy</button>
                  <button id="btnExportar" className="btn" title="Exportar citas a CSV" onClick={handleExport}>­ƒôñ Exportar</button>
                </div>
              </div>
