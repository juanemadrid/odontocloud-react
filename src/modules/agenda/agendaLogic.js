...
export function initAgendaCalendar(db, auth) {
          ...
          <tr class="fade-in-row" ${dataEmailAttr}>
            ...
            <td>
              <button class="btn-whatsapp" data-cel="${
                r.telefono
              }" style="padding:4px 6px;font-size:13px;cursor:pointer;">📲 WhatsApp</button>
            </td>
          </tr>`;
      }
  ...
  function updateOcupacion(bookedCount) {
    ...
    ocupacionInfo.textContent = `Ocupación: ${bookedCount} / ${SLOTS_PER_DAY} (${
    ...
    btnNuevaCita.addEventListener("click", () => {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          alert(
            "❌ Error al guardar la cita. Revisa la consola para más detalles."
          );
        }
      });
    });
