...
export default function NotaDebitoForm({ onCancel, onSuccess }) {
    ...
    const [generarSaldoContra, setGenerarSaldoContra] = useState(true);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setError("Error al guardar la Nota de Débito. Revisa la consola.");
        } finally {
            setSaving(false);
        }
    };
        ...
        <div className="p-4 md:p-6 max-w-[1400px] mx-auto space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-white p-6 rounded-[28px] border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all">
                <div className="flex items-center gap-4">
                    <button 
                        onClick={onCancel}
                        className="w-10 h-10 flex items-center justify-center hover:bg-slate-50 rounded-xl transition-all text-slate-400 hover:text-orange-500 border border-transparent hover:border-slate-100 active:scale-95 group"
                        title="Volver"
                    >
            ...
            {success && (
                <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-[24px] flex items-center gap-4 animate-in zoom-in">
                    ...
                    <div>
                        <h4 className="text-emerald-800 font-black uppercase text-sm">¡Nota de Débito Guardada!</h4>
                        <p className="text-emerald-600 text-xs font-medium uppercase tracking-wide">El documento y saldo en contra se han registrado con éxito.</p>
                    </div>
                </div>
            )}
            ...
            <div className="space-y-6">
                    ...
                    <div className="p-4">
                        {loading ? (
                            <div className="p-10 text-center flex flex-col items-center justify-center">
                                <div className="w-8 h-8 border-4 border-rose-500/20 border-t-orange-500 rounded-full animate-spin mb-2" />
                            ...
                            <table className="w-full text-left">
                                <tbody>
                                                ...
                                                <div className="absolute left-0 right-0 top-12 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 overflow-hidden max-h-60 overflow-y-auto">
                                                    {filteredPatients.map(p => (
                                                        <div
                                                            ...
                                                            className="px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-orange-500 transition-colors cursor-pointer border-b border-slate-50 last:border-0"
                                                        >
                                    ...
                                    <FormRow label="Notas u observaciones">
                                        <textarea
                                            placeholder="Detalle el motivo o notas adicionales de la Nota de Débito..."
                                            value={observaciones}
                                ...
                                <table className="w-full text-left text-xs border-collapse">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        ...
                                        <tr className="bg-slate-50/50 font-black border-t-2 border-slate-100">
                                            ...
                                            <td className="py-4 px-4 text-right text-orange-500 text-base">{fmt(totalCalculado)}</td>
                                            <td></td>
                                        </tr>
                                    </tbody>
                                </table>
                ...
                {/* Saldo en Contra Section */}
                <div className="bg-white rounded-[28px] border border-slate-100 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-100">
                        <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest">Saldo en contra</h3>
                    </div>
                    <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="space-y-1">
                            <h4 className="text-xs font-black text-slate-700 uppercase tracking-wider">Generar saldo en contra para el tercero</h4>
                            <p className="text-[11px] text-slate-400 font-medium">Si se activa, el valor total se cargará como una deuda en la ficha financiera del paciente.</p>
                        </div>
                        <div className="flex items-center gap-6">
                            <div className="flex flex-col text-right">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Valor a generar</span>
                                <strong className="text-orange-500 text-lg font-black">{generarSaldoContra ? fmt(totalCalculado) : fmt(0)}</strong>
                            </div>
                            <button
                                type="button"
                                onClick={() => setGenerarSaldoContra(!generarSaldoContra)}
                                className={`w-14 h-8 flex items-center rounded-full p-1 transition-all duration-300 ${
                                    generarSaldoContra ? "bg-[#8cc33f]" : "bg-slate-200"
                                }`}
                            >
                                <div
                                    className={`bg-white w-6 h-6 rounded-full shadow-md transform transition-all duration-300 ${
                                        generarSaldoContra ? "translate-x-6" : "translate-x-0"
                                    }`}
                                />
                            </button>
                        </div>
