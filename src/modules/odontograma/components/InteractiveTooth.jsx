...
export default function InteractiveTooth({ 
    ...
    const renderGeneralEffects = () => {
        if (isExtraccion) {
            return (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
                    <svg viewBox="0 0 24 24" className="w-[65%] h-[65%] text-red-500 drop-shadow-sm" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round">
        ...
        if (g === 'fractura') {
            return (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
                    <svg viewBox="0 0 24 24" className="w-[55%] h-[55%] text-purple-500 drop-shadow-sm" fill="currentColor">
        ...
        if (isImplante) {
            ...
            return tool ? (
                <div className="absolute inset-0 flex items-center justify-center text-cyan-500 opacity-80 pointer-events-none z-20 p-2">
                    {tool.icon}
                </div>
            ) : null;
        }
